...
<xsl:template name="WriteBody">
    <xsl:choose>
        <xsl:when test="$maintree = '1'">
            <xsl:call-template name="WritePath" />
            <xsl:for-each select="text[@zone = 'main' and @tree = 'true']">
                <a name="{@id}"></a>
                <xsl:if test="title">
                ...
                <p class="post-footer align-right">
                    <a href="/{tid}.html" class="comments">Comments (7)</a>
            ...
            <xsl:for-each select="text[@zone = 'main']">
                ...
                <p pasobject="text">
                    <xsl:if test="shorttext">
                        <xsl:value-of select="shorttext" disable-output-escaping="yes" /><br />
                    </xsl:if>
                </p>
                <p class="post-footer align-right">
                    <a href="/{tid}.html" class="readmore">Read more</a>
                    <a href="/{tid}.html" class="comments">Comments (7)</a>
                    <span class="date"><script type="text/javascript">document.write(time2date(<xsl:value-of select="@date" />));</script></span>	
                </p>
            </xsl:for-each>
